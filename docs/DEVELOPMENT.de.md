...
Läuft auf dem Host bereits anderes, vor dem ersten Start auf Port-Konflikte prüfen:

```bash
./scripts/lumio-check-ports.sh
```

Details siehe [docs/PORT-CHECK.de.md](PORT-CHECK.de.md).
...
- **Browser-Upload schlägt fehl mit "DNS error: minio"** → betrifft nur Versionen vor v0.51.0 (seitdem signiert die API ohne `S3_PUBLIC_URL` automatisch auf `http://<request-host>:9000`). Falls es dennoch auftritt: `S3_PUBLIC_URL` zeigt auf den internen Container-Hostname — auf die öffentliche S3-Subdomain setzen (`https://s3.galerien.example.com`) oder entfernen.
