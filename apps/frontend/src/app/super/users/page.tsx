  ...
  return (
    <div className="px-4 sm:px-8 py-6 max-w-5xl">
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-semibold">{t("super.usersTitle")}</h1>
          <p className="text-ui-sm text-ink-tertiary mt-0.5">
            {t("super.usersSubtitle")}
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        <input
          ...
          placeholder={t("super.usersSearchPlaceholder")}
        ...
        >
          <option value="">{t("super.usersAllRoles")}</option>
        ...
        <select
          ...
          className="h-9 px-2 rounded-md border border-line-subtle bg-surface-raised text-ui-sm"
        >
          <option value="">{t("super.usersAllStatuses")}</option>
          <option value="active">{t("super.usersStatusActive")}</option>
          <option value="invited">{t("super.usersStatusInvited")}</option>
        ...
        >
          <option value="">{t("super.usersAllTenants")}</option>
          {tenants.map((t) => (
            ...
            <thead className="text-ink-tertiary text-ui-xs uppercase tracking-wide">
              <tr className="border-b border-line-subtle">
                <th className="text-left font-medium px-3 py-2">
                  {t("super.usersColUser")}
                </th>
                <th className="text-left font-medium px-3 py-2">
                  {t("super.usersColTenant")}
                </th>
                <th className="text-left font-medium px-3 py-2">
                  {t("super.usersColRole")}
                </th>
                <th className="text-left font-medium px-3 py-2">
                  {t("super.usersColStatus")}
                </th>
                <th className="text-right font-medium px-3 py-2">
                  {t("super.usersColAction")}
                </th>
              </tr>
            </thead>
            <tbody>
...
function EmailUserDialog({
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <h2 className="text-lg font-semibold">{t("super.usersMailTitle")}</h2>
...
function AnnouncementUserDialog({
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <h2 className="text-lg font-semibold">{t("super.usersBannerTitle")}</h2>
          <p className="text-ui-xs text-ink-tertiary mt-0.5">
            {user.email} · {user.tenant.name} ({user.tenant.slug})
          </p>
        </div>
          ...
          <>
            ...
            <div className="flex gap-3">
              <div className="flex-1">
                ...
                >
                  <option value="info">{t("super.usersBannerLevelInfo")}</option>
                  <option value="warning">{t("super.usersBannerLevelWarning")}</option>
                  <option value="critical">{t("super.usersBannerLevelCritical")}</option>
                </select>
              </div>
              <div className="flex-1">
                <label className="text-ui-xs text-ink-secondary block mb-1.5">
                  {t("super.usersBannerExpires")}
                </label>
                <select
                  value={expiry}
                  onChange={(e) =>
                  ...
                  className="w-full h-9 px-2 rounded bg-surface-sunken border border-line-subtle text-ui text-ink-primary focus:outline-none focus:border-accent"
                >
                  <option value="7">{t("super.usersBannerExpires7")}</option>
                  <option value="30">{t("super.usersBannerExpires30")}</option>
                  <option value="none">{t("super.usersBannerExpiresManual")}</option>
                </select>
              </div>
            </div>
            {severity !== "critical" && (
              <label className="flex items-center gap-2 text-ui-sm text-ink-secondary">
                <input
                  ...
                  disabled={busy}
                />
                {t("super.usersBannerDismissible")}
              </label>
            )}
            {error && (
              <p className="text-ui-sm text-semantic-danger">{error}</p>
...
function EditUserDialog({
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <h2 className="text-lg font-semibold">{t("super.usersEditTitle")}</h2>
          <p className="text-ui-xs text-ink-tertiary mt-0.5">
            {user.email} · {user.tenant.name} ({user.tenant.slug})
          </p>
        </div>
        ...
        {user.status === "invited" ? (
          ...
          <div>
            ...
            >
              <option value="active">{t("super.usersStatusActive")}</option>
              <option value="disabled">{t("super.usersStatusDisabled")}</option>
            </select>
          </div>
        )}
  ...
  return (
    ...
    >
      <form
        ...
        className="w-full max-w-md bg-surface-raised border border-line-subtle shadow-2xl rounded-lg p-6 space-y-4"
      >
        <h2 className="text-lg font-semibold">{t("super.usersCreateTitle")}</h2>
        <p className="text-ui-xs text-ink-tertiary -mt-2">
          {t("super.usersCreateHint")}
        </p>

        <div>
          <label htmlFor="c-tenant" className="text-sm font-medium block mb-1">
            {t("super.usersCreateTenant")}
          </label>
          <select
            id="c-tenant"
            value={tenantId}
            ...
            className="w-full rounded-md border border-line-subtle px-3 py-2 text-sm bg-surface-raised"
          >
            {tenants.length === 0 && <option value="">{t("super.usersCreateNoTenant")}</option>}
            {tenants.map((t) => (
              <option key={t.id} value={t.id}>
                {t.displayName ?? t.name} ({t.slug})
              </option>
